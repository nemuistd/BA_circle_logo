...
- 最新の名刺メーカー機能に合わせて、`README.md`、`usage.html`、`HANDOFF.md`、`DESIGN_SYSTEM.md`、`DESIGN_PROPOSAL.md`、`LOGO_PROPOSAL.md`、`LOGO_CANDIDATE_REVIEW.md` を更新した。
- ローカル確認は `start-test-server.bat` 経由、主ロゴカテゴリ分離、サブロゴ選択、SVGサブロゴ色変更、PNG/SVG保存の説明を追記した。
- 公開利用者向けの `usage.html` から、開発者向けのローカルサーバー記述を外した。

## 2026-05-15

- `index.html` に `assets/sub_logo` 配下のサブロゴ選択UIを追加し、既存の連邦捜査部ロゴに加えてシャーレ系素材をサブロゴとして選べるようにした。
- `assets/sub_logo/シャーレ.svg` はサブロゴ色をカラーパレット、カラーピッカー、HEX入力で変更し、プレビューとSVG/PNG保存に反映するようにした。
- `start-test-server.bat` を追加し、ローカルテスト用に `index.html` を簡単にHTTP配信できるようにした。
- 色変更可能なサブロゴSVGを名刺SVGへ埋め込む際に、元SVGの穴抜き描画ルールを保つようにして、輪郭内側が塗りつぶされないようにした。
- `assets/logo/別ver.png` を主ロゴのEX候補に追加し、削除済みの `シャーレロゴ_灰.png` と `シャーレロゴ_黒.png` をサブロゴ候補から外した。
- 主ロゴのカテゴリに `サブロゴ` タブを追加し、連邦捜査部系・透過系の素材を `EX` から分離した。
- サブロゴ色変更時にサブロゴ一覧全体を再描画せず、SVGサムネイルだけ更新するようにして、画面の点滅とスクロールジャンプを抑えた。

## 2026-05-11

- `usage.html` を追加し、公開ページ内で名刺メーカーの基本操作、色設定、ロゴ選択、光輪・サブロゴ調整、PNG/SVG保存手順を確認できるようにした。
- `index.html` のトップバーに「使い方」リンクを追加し、`README.md` の入口一覧にも `usage.html` を追記した。
- `usage.html` の説明順を、最短の作成手順、困った場合、こだわり調整機能の紹介へ整理した。

- `index.html` のロゴ選択に `assets/logo` 配下のEXロゴを追加し、追加カテゴリ名を `EX` に変更した。
- テキスト/フォント設定に折りたたみ式の色設定を追加し、サークル名、英字コピー、ID、所属表記、名前の色を候補色、カラーピッカー、HEX入力で変更できるようにした。
- 設定項目の並びを名刺上の読み順に寄せ、IDと所属表記を同じ行で編集できるようにした。
- 主ロゴに重ねる色変更可能な光輪レイヤーを追加し、`ロゴ透過_下のみ.png` と組み合わせて使えるようにした。
- 光輪の塗りと輪郭を同じ座標系で描画し、`ロゴ透過_下のみ.png` の 705x590 キャンバスに合わせて位置を調整した。
- `連邦捜査部ロゴ_copyright_Nexon,Yostar.png` をサブロゴとして表示できるようにし、サイズ、左右位置、上下位置を主ロゴとは独立して調整できるようにした。
- SVG/PNG保存に文字色、主ロゴ、光輪、サブロゴの状態を反映するようにした。
- ブラウザで最終確認済みの状態を `v1.1` リリース候補とした。
