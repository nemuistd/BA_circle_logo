...
GitHub Pagesで公開すると、各メンバーがブラウザ上で名刺を編集して画像保存できます。ローカルで確認する場合は、`file://` でHTMLを直接開くのではなく、簡易ローカルサーバー経由で開いてください。直開きではブラウザの制限により、プレビュー画像やPNG保存が正しく動かない場合があります。

ローカル確認は `start-test-server.bat` を実行し、表示された `http://127.0.0.1:8000/index.html` を開きます。別ポートを使う場合は `start-test-server.bat 8765` のように指定できます。
...
- サブロゴ素材: `assets/sub_logo/*`
- 透過版contact sheet: `assets/generated/透過/*.png`
- ローカルテスト用サーバー: `start-test-server.bat`
...
- ロゴ選択は `シンプル`、`ケーキ simple`、`ケーキ rich`、`旧 ornate`、`サブロゴ`、`EX` のカテゴリで切り替え可能
- 主ロゴのサイズ、左右位置、上下位置を調整可能
- 主ロゴへ色変更可能な光輪レイヤーを追加可能
- 連邦捜査部ロゴや `assets/sub_logo` 配下の素材をサブロゴとして表示し、サイズと位置を調整可能
- `assets/sub_logo/シャーレ.svg` はサブロゴ色を候補色、カラーピッカー、HEX入力で変更可能
- PNGまたはSVGで保存可能
