...
- `README.md`
  - 起動方法と入口。
  - ローカル確認は `start-test-server.bat` 経由を推奨。
- `start-test-server.bat`
  - `python -m http.server` でこのディレクトリを配信するローカルテスト用バッチ。
  - 既定URLは `http://127.0.0.1:8000/index.html`。引数でポート変更可。
- `デザイン要件.txt`
  - このプロジェクトの正となる要件。
- `index.html`
  - 実用寄りの縦長名刺メーカー。
  - 先生名、サークル名、英字コピー、所属表記、IDを入力し、PNG/SVG保存できる。
  - 主ロゴ、光輪、サブロゴの表示、サイズ、位置、色を調整できる。
- `usage.html`
  - 名刺メーカーの操作手順と調整機能の説明ページ。
- `logos.html`
  - 旧SVGロゴ試作の比較ページ。
  - 現在は探索用で、最終候補の中心ではない。
...
10. `index.html` を単一の縦長名刺メーカーへ整理し、文字色、主ロゴ選択、光輪、サブロゴ、PNG/SVG保存を一画面で調整できるようにした。
11. `assets/logo` 配下のEX素材、`assets/sub_logo` 配下のサブロゴ素材を選択対象に追加した。
12. `assets/sub_logo/シャーレ.svg` は白地を残し、色付き部分だけを指定色に変更できる。
13. 主ロゴ一覧では、連邦捜査部系・透過系素材を `サブロゴ` カテゴリへ分離し、`EX` には `shiki-logo.png` と `assets/logo/別ver.png` を置いている。
14. `start-test-server.bat` を追加し、ローカルでも保存機能を確認しやすくした。

## 現在の重要な状態

名刺メーカー `index.html` が現時点の実利用画面。ローカルで確認する場合は `start-test-server.bat` を使い、`file://` 直開きは避ける。直開きではブラウザ制限により、SVG/PNG保存や画像読み込みが不安定になることがある。
...
- `assets/logo/連邦捜査部ロゴ_copyright_Nexon,Yostar.png`
- `assets/sub_logo/シャーレ.svg`
- `assets/sub_logo/シャーレ_ハンコ.png`

`シャーレ.svg` は色変更可能。元SVGの `fill-rule:evenodd` を名刺SVGへの埋め込み時にも維持して、穴抜き部分が塗りつぶされないようにしている。色変更時はサブロゴ一覧全体を再描画せず、SVGサムネイルだけ差し替えることでスクロールジャンプを抑えている。
...
1. `start-test-server.bat` で `index.html` を開き、主ロゴ、サブロゴ、色変更、PNG/SVG保存を実ブラウザで確認する。
2. `logo-candidate-review.html` をブラウザで開き、`CSR1` から `CSR12` を 128 / 64 / 32 / 24px で確認する。
3. `LOGO_CANDIDATE_REVIEW.md` の `CS1` から `CS12` の評価を、再生成版 `CSR1` から `CSR12` に置き換える。
4. 優先候補を `CSR4`、`CSR8`、`CSR12`、`S5`、`S7`、`S12` あたりから再選定する。
5. 絞った候補を `index.html` の主ロゴ候補へ差し込み、縦長名刺上で相性を見る。
